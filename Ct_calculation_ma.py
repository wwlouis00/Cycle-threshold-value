...
# global variable
raw_file_path = "./chartDrawing/detection.csv"
ifc_file_path = "./chartDrawing/cali_factor.csv"
...
def normalize(baseline_begin, baseline_end):
    for i in range(0, 16):
        ...
        df_normalization[f'well{i + 1}'] = (df_raw[f'well{i + 1}'] - baseline) / df_current_ifc[
            0]  # normalized = (IF(t)-IF(b))/IFc
...
def ct_calculation(baseline_begin, baseline_end):
    ...
    Csv_well = take_photo()
    print(Ct_value)
    save_excel = pd.DataFrame(
        {"well_1": [Ct_value[0]], "well_2": [Ct_value[1]], "well_3": [Ct_value[2]], "well_4": [Ct_value[3]],
